...
sudo docker run -it --env-file unify-env.list -v $INPUT_FROM_PUMP_DIR:$INPUT_DIR -v $WORKING_DIR_HOST:$WORKING_DIR -v $REF_DIR_HOST:$REF_DIR quay.io/broadsword/recount-unify:1.0.2 /bin/bash -c "source activate recount-unify && /recount-unify/workflow.bash"
